...
uint8_t rx_buffer[0x1000];//  __attribute__ ((section (".usart_mini_rx_buffer"), used));
...
void USART3_IRQHandler(void)
{
	...
	{
		rx_buffer[rx_pos_write % sizeof(rx_buffer)] = USART_ReceiveData(USART);
		rx_pos_write++;
	}

	if (USART_GetITStatus(USART, USART_IT_ORE_RX) != RESET) {USART_ReceiveData(USART); rx_errors++;};
//	if (USART_GetITStatus(USART1, USART_IT_ORE_ER) != RESET) {send('2'); };
//	if (USART_GetITStatus(USART1, USART_IT_NE    ) != RESET) {send('3'); };
//	if (USART_GetITStatus(USART1, USART_IT_FE    ) != RESET) {send('4'); };
//	if (USART_GetITStatus(USART1, USART_IT_PE    ) != RESET) {send('5'); };
}
#endif
